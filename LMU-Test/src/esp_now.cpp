...
volatile bool brakeTriggered = false; // set by main when pothole detected
volatile bool slowSpeedTriggered = false; // set by main when speed is low - low than 5kmh -
...
// Main can call this when pothole is detected; treated as a brake event
void espnow_mark_brake() {
  brakeTriggered = true;
}

// Main can call this when speed is low; treated as a slow speed event
void espnow_mark_slow_speed() {
  slowSpeedTriggered = true;
}
...
void espnow_loop() {
  ...
  // If main marked a brake event (e.g., pothole detected), broadcast once
  if (brakeTriggered) {
    brakeTriggered = false;
    sendBroadcastAndShow("Brake ON");
  }

  // If main marked a slow speed event, broadcast once
  if (slowSpeedTriggered) {
    slowSpeedTriggered = false;
    sendBroadcastAndShow("Bump! Slow down");
  }

  // Old simulation logic retained for reference (disabled to avoid blocking)
  // bike_break = random(0, 2);
  // bike_speed = random(0, 51);
  // if (bike_break) {
  //   sendBroadcastAndShow("Brake ON");
  // }
